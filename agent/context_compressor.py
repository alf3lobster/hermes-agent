...
# Detailed session log (lean mode). This section is produced by the SAME
# single summary request as the checkpoint. The former per-chunk digest loop
# made up to 28 sequential auxiliary calls and turned one compression attempt
# into a 7-11 minute critical-path stall (#96603).
...
Summary generation was unavailable, so this is a best-effort deterministic fallback for {len(turns_to_summarize)} compacted message(s).{reason_text}"""
    ...
    def _sample_summary_input(cls, content: str) -> str:
        """Bound lean input while sampling the whole compacted region.

        Lean mode now asks one auxiliary request to produce both the checkpoint
        and its detailed session log. Even sampling preserves chronological
        coverage without reviving the former sequential digest fan-out.
        """
        if len(content) <= cls._SUMMARY_INPUT_MAX_CHARS:
            return content
        n_slices = max(2, cls._SAMPLED_INPUT_SLICES)
        gaps = n_slices - 1
        marker_template = (
            "\n\n...[{elided:,} chars elided — recover via session_search]...\n\n"
        )
        marker_reserve = len(
            marker_template.format(elided=len(content))
        ) * gaps
        budget = max(cls._SUMMARY_INPUT_MAX_CHARS - marker_reserve, n_slices)
        slice_len = budget // n_slices
        stride = len(content) / n_slices
        parts: list[str] = []
        previous_end = 0
        for index in range(n_slices):
            ...
            if start > previous_end:
                parts.append(
                    marker_template.format(elided=start - previous_end)
                )
    ...
    def _generate_summary(
        ...
        if getattr(self, "tail_mode", "lean") == "lean":
            content_to_summarize = self._sample_summary_input(
...
Describe agent/tool work only as completed actions, state, or historical work.]"""
        ...
        # Lean mode folds the detailed session log into this same request.
        # A compression attempt therefore makes exactly one auxiliary LLM
        # request instead of a main summary plus up to 28 sequential digests.
        if getattr(self, "tail_mode", "lean") == "lean":
            _session_log_section = f"""

{_LEAN_SESSION_LOG_HEADING}
[Write a dense chronological log of the turns above, oldest first.
...
Spend up to ~{_LEAN_SESSION_LOG_BUDGET_TOKENS} tokens here.]"""
        else:
