...
collect_sandbox_logs() {
  ...
  # npm --silent can exit non-zero with an empty captured stderr while writing
  # the causal network/registry error only to ~/.npm/_logs. Preserve those
  # diagnostics beside the proxy/slirp logs instead of reducing CI to
